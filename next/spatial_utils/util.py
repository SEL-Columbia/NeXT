...
class QuadTree:
	...
	def __init__(self, obj, pt_fun=None):
            self.obj = obj
            if (pt_fun):
                setattr(self, '__getitem__', types.MethodType(lambda self, i: pt_fun(self.obj)[i], self))
	    else:
                setattr(self, '__getitem__', types.MethodType(lambda self, i: self.obj[i], self))
